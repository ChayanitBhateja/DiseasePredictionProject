...
const createUser = async (userData) => {
  ...
  const data = await User.findOne({$or:[{ email: userData.email},{userName:userData.userName}] ,isDeleted:false});
...
const verifyEmailToken=async(tokenData)=>{
  if (tokenData.role === USER_TYPE.USER) {
    const userValue= await User.findByIdAndUpdate(tokenData.user, {
      isVerified: true,
    });
    const tokenvalue = await Token.findByIdAndUpdate(tokenData._id, {
      isDeleted: true,
