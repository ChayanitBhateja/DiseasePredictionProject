...
import { adminBlockPatient, approveDoctor, blockUser, deletePatient, deleteUser, getAllDoctor, getAllPatient, user } from "../../services/AuthService";
...
const AllPatientList = () => {
    ...
    function allPatient(search, limit, pageNumber) {
        getAllPatient(search, limit, pageNumber).then((response) => {
            console.log(response, "all getAllPatient")
            setUsers(response.data.data.user);
            const total = response.data.data.count;
            setpageCount(Math.ceil(total / limit));
        });
    }
    function deletePatientByAdmin(id) {
    ...
    function adminBlock(id) {
        ...
        adminBlockPatient(id, toggle).then((response) => {
    ...
    useEffect(() => {
        // setLoader(true);
        if (search.length > 0) {
            allPatient(search, limit, 0)

        } else {

            allPatient(search, limit, pageNumber)
        }
